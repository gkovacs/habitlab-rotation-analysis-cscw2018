...
datadays$condition <- factor(datadays$condition, labels=c("same", "random"))
summary(datadays)
```
...
```{r}
results <- lmer(log_time_spent ~ impression_idx + is_first_visit_of_day + intervention + (1|install_id), data = data_facebook_same)
resultsnull <- lmer(log_time_spent ~ is_first_visit_of_day + intervention + (1|install_id), data = data_facebook_same)
anova(resultsnull, results)
summary(results)
```
