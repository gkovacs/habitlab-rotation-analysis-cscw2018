...
ndata <- subset(datadays_nofirstlast, domain == 'www.facebook.com' | domain == 'www.youtube.com' | domain == 'www.reddit.com' | domain == 'twitter.com' | domain == 'vk.com')
ndata <- subset(ndata, condition == 'same' | condition == 'random')
#summary(ndata)

results <- lmer(log_time_spent ~ days_since_install + condition + (1|domain) + (1|install_id), data = ndata)
resultsnull <- lmer(log_time_spent ~ (1|domain) + (1|install_id), data = ndata)
anova(resultsnull, results)
summary(results)

stargazer(results, 
          covariate.labels = c("Number of days since install", "Condition", "(Intercept)"), 
          dep.var.labels=c("Log time spent per day"), 
          no.space=TRUE, 
          title="Controlling for condition, interventions decline in effectiveness over time, as seen with increasing log time per day.", 
