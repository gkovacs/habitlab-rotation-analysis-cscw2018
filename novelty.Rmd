...
Is there an effect of condition on whether they attritioned?

```{r}
# does not converge after 10000 evaluations
#results <- glmer(attritioned ~ condition + domain/intervention + (1|install_id), family=binomial(link='logit'), data = datadays)
#resultsnull <- glmer(attritioned ~ domain/intervention + (1|install_id), family=binomial(link='logit'), data = datadays)
...
```{r}
# excessively complex model, doesn't converge
#results <- glmer(attritioned ~ condition + intervention + (1|install_id), family=binomial(link='logit'), data = data)
#resultsnull <- glmer(attritioned ~ intervention + (1|install_id), family=binomial(link='logit'), data = data)
#anova(resultsnull, results)
#summary(results)
```
