...
if (!file.exists(basepath)) {
  basepath <- '/Users/golrokh/Desktop/habitlab_data_analysis/data/'
}
...
data_filename <- paste(basepath, 'AUG8.csv', sep='')
...
```{r}
print('onboarding completion ratios')
no_difficulty_selection <- subset(data, difficulty_selection_screen == 'none')
completed <- length(which(no_difficulty_selection$onboarding_completed == 'true'))
not_completed <- length(which(no_difficulty_selection$onboarding_completed == 'false'))
ratio <- completed / (completed + not_completed)
cat('no difficulty selection screen', ratio, '\n')

optional_difficulty <- subset(data, difficulty_selection_screen == 'nodefault_optional')
completed <- length(which(optional_difficulty$onboarding_completed == 'true'))
not_completed <- length(which(optional_difficulty$onboarding_completed == 'false'))
ratio <- completed / (completed + not_completed)
cat('optional difficulty selection screen', ratio, '\n')

forced_difficulty <- subset(data, difficulty_selection_screen == 'nodefault_forcedchoice')
completed <- length(which(forced_difficulty$onboarding_completed == 'true'))
not_completed <- length(which(forced_difficulty$onboarding_completed == 'false'))
ratio <- completed / (completed + not_completed)
cat('forced difficulty selection screen', ratio, '\n')
```

```{r}
print('forced choice vs optional')
ndata <- subset(data, difficulty_selection_screen == 'nodefault_optional' | difficulty_selection_screen == 'nodefault_forcedchoice')
tb <- table(ndata$difficulty_selection_screen, ndata$onboarding_completed)
tb <- tb[which(rowSums(tb) > 0),]
fisher.test(tb)

print('no choice vs optional')
ndata <- subset(data, difficulty_selection_screen == 'none' | difficulty_selection_screen == 'nodefault_optional')
tb <- table(ndata$difficulty_selection_screen, ndata$onboarding_completed)
tb <- tb[which(rowSums(tb) > 0),]
fisher.test(tb)

print('forced choice vs no choice')
ndata <- subset(data, difficulty_selection_screen == 'none' | difficulty_selection_screen == 'nodefault_forcedchoice')
tb <- table(ndata$difficulty_selection_screen, ndata$onboarding_completed)
tb <- tb[which(rowSums(tb) > 0),]
fisher.test(tb)
```
...
```{r}
ndata <- subset(data,  first_active < 20)
ndata <-subset(ndata, difficulty_selection_screen == 'nochoice_easy' | difficulty_selection_screen == 'nochoice_medium' | difficulty_selection_screen == 'nochoice_hard' | difficulty_selection_screen == 'nochoice_nothing' | (difficulty_selection_screen == 'nodefault_optional'))
...
```{r}
ndata <- data
ndata <- subset(ndata, difficulty_selection_screen== 'nodefault_forcedchoice' |  difficulty_selection_screen== 'nodefault_optional' |  difficulty_selection_screen== 'none' )
#ndata <- subset(ndata, did_user_change_interventions_5min == FALSE)
#ndata <- subset(ndata, saw_multiple_facebook_interventions == TRUE)
#ndata <- subset(ndata, saw_multiple_interventions == TRUE)
summary(ndata)
